-- =============================================
-- SEED DATA FOR LAND ADMINISTRATION SYSTEM
-- =============================================
...
-- Sample Land Parcels
INSERT INTO land_admin.land_parcels (parcel_number, name, province_id, district, location_description, area_hectares, area_sqm, coordinates, status, parcel_type, valuation_amount) VALUES
...
INSERT INTO land_admin.land_board_meetings (meeting_number, meeting_date, meeting_time, location, status, published_in_gazette, gazette_date) VALUES
...
-- Sample Events
INSERT INTO land_admin.events (title, description, event_type, start_date, end_date, location, attendees) VALUES
...
-- =============================================
-- NOTE: Sample Leases Commented Out
-- =============================================
-- Leases require a real user from auth.users table
-- Once you create users in your app, you can create leases manually
-- or run this SQL with a real user ID:
--
-- INSERT INTO land_admin.leases (
--   lease_number, parcel_id, leaseholder_id, lease_type,
--   start_date, end_date, duration_years, annual_rent,
--   rent_reduction_percentage, status
-- ) VALUES (
--   'L-2024-00001',
--   (SELECT id FROM land_admin.land_parcels WHERE parcel_number = 'P-2025-0002'),
--   'YOUR_REAL_USER_ID_HERE',
--   'residential_high', '2024-06-01', '2123-06-01', 99, 15000, 25.0, 'active'
-- );
...
CREATE OR REPLACE VIEW land_admin.application_summary AS
SELECT
  a.id,
...
CREATE OR REPLACE VIEW land_admin.active_leases_summary AS
SELECT
  l.id,
  l.lease_number,
  l.lease_type,
...
CREATE OR REPLACE VIEW land_admin.dashboard_statistics AS
SELECT
...
CREATE OR REPLACE VIEW land_admin.monthly_application_stats AS
SELECT
...
-- Success message
SELECT 'Sample data loaded successfully into land_admin schema!' AS message,
       (SELECT COUNT(*) FROM land_admin.land_parcels) AS parcels_loaded,
       (SELECT COUNT(*) FROM land_admin.applications) AS applications_loaded,
       (SELECT COUNT(*) FROM land_admin.events) AS events_loaded;
